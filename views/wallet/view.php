...
<div class="wallet-content-item">
    <div class="wallet-balance-container">
        ...
        <div class="transfer-area">
            <button class="styled-button-success transfer-collapse-button collapsed" type="button" data-toggle="collapse" data-target="#transfer-details" aria-expanded="false" aria-controls="transfer-details">
                Transfer
            </button>
        </div>
    </div>
        ...
        <div class="jumbotron">
            <h3>
                <?php if ($wallet->isFeeWallet)
                    echo 'Lightning Network routing fees incurred from using this node. You will see a negative balance.';
                else if ($wallet->isKeysendWallet) {
                    echo 'Inbound keysend payments to this node that do not have a mapping to a wallet are collected here';
                }
                ?>
            </h3>
        </div>
...
<div class="collapse" id="deposit-details" style="margin-top: 25px;">
    ...
    <div class="wallet-content-item row" style="padding: 25px; margin-bottom: 12px; box-shadow: 0 1px 3px hsla(0, 0%, 0%, .2);">
        <div class="col-md-6">
            ...
            <?php
                $key = $wallet->getFirstAccessKeyByRole(\app\behaviors\UserAccessKeyBehavior::ROLE_WALLET_INVOICE);
                $userKey = Yii::$app->user->identity->getFirstAccessKeyByRole(\app\behaviors\UserAccessKeyBehavior::ROLE_PUBLIC_API_KEY);
               $this->registerJs("
