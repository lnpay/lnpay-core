...
<div class="integration-webhook-form col-md-12 well">
    ...
    <?php
        ...
        $actions = \app\models\action\ActionName::getActiveWebhookArrayByType();
    ...
    <?php  echo $form->field($model, 'status_type_id')->dropDownList(\yii\helpers\ArrayHelper::map(\app\models\StatusType::getAvailableStatuses(\app\models\StatusType::TYPE_WEBHOOK),'id','display_name')); ?>
...
<?php if ($model->id) { ?>
    <div class="integration-webhook-form col-md-6 well">
        <?php
        $webhookTestForm = new \app\models\integration\WebhookTestForm();
        $webhookTestForm->integration_webhook_id = $model->external_hash;
        $events = \yii\helpers\ArrayHelper::map(\app\components\ActionComponent::getAvailableTestActionObjects(),'name','display_name');
